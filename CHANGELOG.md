...
1. **Schema collapse.** Renumbered around two new reserved slots, folded old Decisive Players + the style-fit detail table into a "Read" column on the Shape table, added a Fix column to What This Squad Cannot Do, merged the Wage Bill and Squad Audit's value-created figure into The Money, and added a hard word budget (2,000-2,500 whole briefing, no section over 350).
2. **Decision board + sequencing.** New Sections 2-3: a Call/Who/Number/Trigger/Why table assembled entirely from existing data (exit candidates, recruitment priorities, decisive players, squad-audit mismatches), and a default-ordered sequencing list. Caught and fixed a contradiction while verifying: a player who was both an exit candidate and had a playing-time mismatch was getting both a Sell row and a Fix row.
3. **Un-isolated the Target Dossier.** Section 9 (Targets) now shows a combined lead-candidate table (one row per recruitment priority, one per exit-replacement case) right next to the profile that motivated it, instead of 3,000 words away in a separate section — the full candidate detail moves behind a collapsible `<details>` block in HTML output rather than disappearing.
4. **Two correctness joins.** A load-bearing exit now carries a gate ("replacement must be contracted first") that the decision board's Trigger column and the sequencing order both respect; a player tiered Exit/Saleable with a league style-fit percentile ≥70 gets flagged as a conflict in Housekeeping. Caught a second contradiction while testing: a gated exit was producing both a Sell row and a Hold row for the same player.
5. **edwards.md voice rewrite.** Inverted "diagnostic before prescriptive" to "decision first, evidence after"; added rules for bullet grammar, comparators on every number, thresholds-and-gates on every recommendation, stating each fact once, and flagging low confidence. Cleaned the file split so section order lives in exactly one place (`report.py`'s task instructions) instead of two independently-worded copies that could drift.
6. **html_report.py visual port.** Every chart anchor was still pointing at the old 13-section ids — silently broken since stage 1, since a non-matching anchor no-ops rather than errors. Re-anchored everything, added coloured decision tags, a KPI strip with sub-lines, and percentile bars with a median rule for Section 8 (ported from the reference `report_v2.html`). Caught and fixed an unrelated pre-existing bug while screenshot-verifying: `markdown_to_html` never supported ordered lists, so Section 3's numbered sequence was silently collapsing into one run-on paragraph.
